...
//uint8_t rx_data;
          ...
          lcd_write_cmd(&hi2c1,0X01);             // clear display
          lcd_write_cmd(&hi2c1, 0X0C);                // hide cursor
          lcd_goto_rc(&hi2c1, 0, 0);                            // cursor to first row first col
          lcd_putstr(&hi2c1, "Room Temp Cont");
          HAL_Delay(2000);// print text
          lcd_write_cmd(&hi2c1,0X01);             // clear display
  ...
  while (1)
  {
	  ...
	  int temp =  (adc_val * 100) / 4095;

	  sprintf(buffer,"Temp: %0d\r\n", temp);
